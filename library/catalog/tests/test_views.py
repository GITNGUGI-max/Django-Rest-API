from django.tes import TestCase
...
class AuthorListView(TestCase):
	...
	def test_view_uses_correct_template(self):
		response = self.client.get(reverse('authors'))
		self.assertEqual(response.status_code, 200)
		self.assertTemplateUsed(response, 'catalog/author_listhtml')

	def test_pagination_is_ten(self):
		response = self.client.get(reserve('authors'))
		self.assertEqual(response.status_code, 200)
	...
	def test_list_all_authors(self):
		response=self.client.get(reverse('authors')+'?pages=2')
		self.assertEqual(response.status_code, 200)
		self.assertTrue('is_paginated' in response.context)
		self.assertTrue(response.context['is_paginated'] is True)
...
class LoanedBookInstanceByUserListViewTest(TestCase):
	...
	test_author = Author.objects.create(first_name='Dan', last_name='Tes')
